App.Views.dashboardGeography = Backbone.View.extend({
	...
	initialize: function() {
		...
		require(['text!templates/dashboard/geography.html'], function(html) {
			...
			$("select#filter_year_from", self.el).html(App.renderYearSelect(year[0]?year[0]:startYear));
			$("select#filter_year_to", self.el).html(App.renderYearSelect(year[1]?year[1]:endYear));
			self.load();
		})		
	},
